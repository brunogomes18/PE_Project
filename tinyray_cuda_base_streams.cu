...
__global__ void computeKernel(byte* data,float screen_dim, float aspect_ratio, Vec3 origin, Sphere* spheres, Light* lights, int offset,int streamSize){
    int x = ( offset + (threadIdx.x + blockIdx.x * blockDim.x)) % 1920;
    int y = ( offset + (threadIdx.x + blockIdx.x * blockDim.x)) / 1920;
    int limit = offset + streamSize;
...
int main(void)
{
    ...
    int n= WIDTH*HEIGHT,threadBlockSize= 32;
     ...
     //Streams
    const int nStreams = 4; // Experiment value
    const int streamSize = n / nStreams; // +1 for not divisible cases
    ...
    for(int i = 0; i < nStreams; i++){
        ...
        int streamBytes = HEIGHT*WIDTH - offset;

        checkCudaCall( cudaStreamCreate(&stream[i]) );

        // copy the original vectors to the GPU
        checkCudaCall(cudaMemcpyAsync(deviceLights, lights, sizeof(Light[3]), cudaMemcpyHostToDevice,stream[i]));
        checkCudaCall(cudaMemcpyAsync(deviceSpheres, spheres, sizeof(Sphere[4]), cudaMemcpyHostToDevice,stream[i]));

        // Kernel function
        computeKernel<<<streamSize/threadBlockSize+1, threadBlockSize,0,stream[i]>>>(deviceData,screen_dim,aspect_ratio,origin,deviceSpheres,deviceLights,offset,streamSize);

        // Copy from device to host
        checkCudaCall(cudaMemcpyAsync(&data[offset*3] , &deviceData[offset*3] , streamSize*3*sizeof(byte), cudaMemcpyDeviceToHost,stream[i]));
